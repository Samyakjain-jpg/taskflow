...
initDb().then(database => {
  ...
  const adminId = db.prepare('INSERT INTO users (name, email, password, role, avatar) VALUES (?, ?, ?, ?, ?)').run('Alex Chen', 'admin@demo.com', password, 'admin', 'AC').lastInsertRowid;
  const member1Id = db.prepare('INSERT INTO users (name, email, password, role, avatar) VALUES (?, ?, ?, ?, ?)').run('Sam Rivera', 'member@demo.com', password, 'member', 'SR').lastInsertRowid;

  const p1 = db.prepare('INSERT INTO projects (name, description, color, owner_id, due_date) VALUES (?, ?, ?, ?, ?)').run('Product Redesign', 'UI/UX overhaul', '#6366f1', adminId, '2026-06-30').lastInsertRowid;
